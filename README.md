...
Repositório pessoal de automação de QA para a plataforma Twygo.
Combina o repo oficial `Twygo/twygo-agents-qa` (TypeScript + Playwright) com os testes Python legados, skills e memórias do Claude Code — tudo em um lugar para pegar de qualquer máquina.
...
```bash
git clone https://github.com/DantexgerBR/Twygo-QA-Tester.git
cd Twygo-QA-Tester/agent-playwright
cp .env.example .env        # preencher com credenciais
npm install
npx playwright install chromium
```

## Restaurar memórias do Claude Code

```bash
MEMDIR="$HOME/.claude/projects/$(pwd | tr '/' '-' | sed 's/^-//')/memory"
mkdir -p "$MEMDIR"
cp Twygo-QA-Tester/_claude_memory/* "$MEMDIR/"
cp Twygo-QA-Tester/_claude_global_skills/twygo-test/SKILL.md \
   "$HOME/.claude/skills/twygo-test/SKILL.md" 2>/dev/null || true
```

## Executar suite de marca d'água

```bash
cd agent-playwright
PROJECT=marca-dagua npm run agent:run -- --suite "Marca d'Água em Vídeo"
```
...
```
Twygo-QA-Tester/
├── agent-playwright/       # E2E TypeScript + Playwright (agente principal)
│   └── projects/
│       ├── marca-dagua/    # T-1594..T-1602 — marca d'água em vídeo ✅
│       └── widgets/        # Painéis/Widgets ✅
├── agent-at/               # Análise de Teste — Python
├── agent-tasks-qa/         # Quebra de Atividades — Python
├── agent-db/               # Validação de banco — esqueleto
├── shared/                 # Gotchas cross-agente da plataforma Twygo
├── legacy/
│   └── playwright-python/  # Specs Python originais (referência)
├── _claude_memory/         # Memórias persistentes do Claude Code
└── _claude_global_skills/  # Skills globais (~/.claude/skills/)
```

---

## Original do monorepo oficial
...
|---|---|---|---|---|
| **Tasks QA** | [`agent-tasks-qa/`](agent-tasks-qa/) | Quebra Discovery/Spike em atividades de QA com estimativas (planejamento/refinamento) | Python + Claude Code | ✅ Ativo |
...
| Você precisa de... | Use |
|---|---|
| Quebrar atividades de QA + estimativas para planejamento | [agent-tasks-qa](agent-tasks-qa/) |
...
| **Python** | agentes Python (agent-tasks-qa, agent-at, agent-db) | 3.10+ | https://python.org/ |
...
claude
> /break-qa-tasks                 # após depositar Discovery + planilha de Dev em docs/
```
...
Saída: `agent-tasks-qa/output/QA_Atividades_Widgets_Complementada.xlsx` (Dev+QA)
e `QA_Only_Widgets.xlsx`. Revise estimativas, importe no gestor de projetos
da Twygo.
...
│
├── agent-tasks-qa/                 # Agente upstream — quebra de atividades de QA
│   ├── README.md · CLAUDE.md
│   ├── requirements.txt            # openpyxl, python-docx, docx2txt
│   ├── docs/                       # Discovery + planilha de Dev (input do usuário)
│   ├── output/                     # planilhas .xlsx geradas (gitignored)
│   └── .claude/skills/             # break-qa-tasks, read-inputs, generate-qa-sheet, ...
│
...
│   └── .claude/skills/
│
└── [agent-api, agent-pentest, agent-docs-qa — entram aqui no futuro]
```
...
|---|---|---|
| Tasks-QA (planejamento) | [agent-tasks-qa/README.md](agent-tasks-qa/README.md) | [agent-tasks-qa/CLAUDE.md](agent-tasks-qa/CLAUDE.md) |
