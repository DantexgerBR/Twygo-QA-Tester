 ...
 */
/**
 * **SeedAdminPageBase** — POM de provisionamento de seed genérico Twygo.
 *
 * Cross-projeto: criar curso/trilha/pacote, criar usuário aluno, matricular,
 * publicar curso, configurar atividades, completar curso como aluno,
 * cleanup `*_safe`. Projetos específicos estendem esta classe e adicionam
 * helpers do domínio (ex: `setHasRecertification` em
 * `projects/recertificacao/pages/SeedAdminPage.ts`).
 *
 * **Pré-requisitos pra herança**: o sub-classe vê `profileSwitcher`,
 * `adminContextReady`, e os helpers `private` foram promovidos a
 * `protected` pra reuso.
 *
 * Skill canônica: [[provisionar-seed]] v2.1+ §"Como projeto novo herda
 * o pool".
 */
export class SeedAdminPageBase extends BasePage {
  readonly path = '';
  protected readonly profileSwitcher: ProfileSwitcher;
  protected adminContextReady = false;
   ...
   */
  protected async ensureAdminProfile(): Promise<void> {
   ...
   */
  protected async setHabilitarReinscricao(enabled: boolean): Promise<void> {
   ...
   */
  protected async submitAndWaitForEditUrl(): Promise<void> {
   ...
   */
  protected async findEventRowAndClickKebab(contentName: string): Promise<void> {
