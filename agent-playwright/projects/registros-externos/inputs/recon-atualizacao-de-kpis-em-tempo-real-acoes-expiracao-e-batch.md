...
- **Excluir (modal)**: AT diz "Excluir registro?" + alerta "Esta ação não pode ser desfeita." + botão "Excluir". ⚠️ Em recon dirigido por script, o click no menuitem "Excluir" NÃO abriu modal detectável — candidato a heal (talvez exija `dispatchEvent` como [[navegar-sidebar-admin-twygo]], ou interação real). Validar no audit/heal.
