...
test.describe('Criação de Design de Página', () => {
  test('Acessar criação de Página via menu Adicionar', async ({ page }) => {
    await allure.epic('Twygo - Modelos de conteúdo');
    await allure.feature('Criação de Design de Página');
    await allure.story('Acessar criação de Página via menu Adicionar');
    ...
    await allure.step('3. Clicar "Página" e validar redirect pra criação', async () => {
      await page.locator('[data-test-id="content-models-design-add-page"]').click();
      await expect(page).toHaveURL(/template_designs\/new\?kind=page/, { timeout: 15_000 });
    });
  });
});
