...
test.describe('Mobile', () => {
  test('Visualização do aluno em viewport Mobile 414x896 (vertical)', async ({ page, step }) => {
    test.fixme(
      true,
    ...
    await allure.story('Visualização do aluno em viewport Mobile 414x896 (vertical)');
    await allure.severity('normal');
    await allure.label('executionType', 'manual');

    await step('1. Acessar /play no viewport 414x896', async () => {
