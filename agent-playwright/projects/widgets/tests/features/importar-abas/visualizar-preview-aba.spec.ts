...
test.describe('Importar abas', () => {
  ...
  test('Visualizar preview da aba selecionada', async ({ page, step }) => {
    ...
    await step('3. Selecionar Aba X e verificar preview da aba no modal', async () => {
      ...
      // Preview exibe nome da aba selecionada — escopar via preview pra não casar
      // o label do tab-select (que também tem `<p>Aba X</p>`)
      await expect(preview.getByText('Nome:')).toBeVisible();
      await expect(preview.getByText(data.tabXName, { exact: true })).toBeVisible();

      // Preview exibe quantidade de widgets (2)
      await expect(preview.getByText('Widgets:')).toBeVisible();
      await expect(preview.getByText('2', { exact: true })).toBeVisible();

      // Preview exibe lista dos widgets inclusos — nomes confirmados live via PainelFormPage.WIDGET_IDS
      await expect(preview.getByText('Widgets inclusos:')).toBeVisible();
      await expect(preview.getByText('Resumo de atividades')).toBeVisible();
      await expect(preview.getByText('Conteúdos em andamento')).toBeVisible();
    });
    ...
    await step('5. Verificar campo "Categoria" com opção padrão "Aprendizagem"', async () => {
      // Categoria é <select> HTML (sem data-test-id); locator('select') escopado
      // ao modal resolve unicamente (vs getByRole('combobox') que matcha 3 elementos).
