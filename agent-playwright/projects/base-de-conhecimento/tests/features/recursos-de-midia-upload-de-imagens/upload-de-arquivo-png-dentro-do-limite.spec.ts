...
test.describe('Recursos de Mídia - Upload de imagens', () => {
  ...
  // Requer fixture em test-assets/uploads/images/png-valid.png (~2 MB).
  // Catálogo em test-assets/README.md.
  test('TC1 — Upload de arquivo PNG dentro do limite', async ({ page }, testInfo) => {
    await allure.epic('Twygo - Base de Conhecimento');
    await allure.feature('Recursos de Mídia - Upload de imagens');
    await allure.story('TC1 — Upload de arquivo PNG dentro do limite');
    await allure.severity('critical');

    repoName = `Repositório Upload PNG w${testInfo.workerIndex}-${Date.now()}`;
    ...
    await allure.step('1. Criar repositório', async () => {
      ...
      await formPage.fillDescription('Teste upload PNG');
    ...
    await allure.step('3. Upload de arquivo PNG', async () => {
      const fileInput = page.locator('input[type="file"]').last();
      await fileInput.setInputFiles(shared.paths.png);
      await expect(page.getByText(shared.toastSucesso).first()).toBeVisible({ timeout: 15_000 });
    });

    await allure.step('4. Verificar arquivo na listagem de mídias', async () => {
      await expect(page.getByText(shared.names.png)).toBeVisible();
    });
  });
});
