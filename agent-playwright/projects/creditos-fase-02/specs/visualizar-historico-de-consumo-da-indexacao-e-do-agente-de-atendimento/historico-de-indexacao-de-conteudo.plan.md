# Histórico de indexação de conteúdo
...
#### 1.1. Histórico de indexação de conteúdo

**File:** `tests/features/visualizar-historico-de-consumo-da-indexacao-e-do-agente-de-atendimento/historico-de-indexacao-de-conteudo.spec.ts`
...
**Steps:**
  ...
  3. Verificar que existe ao menos um registro de 'Indexação de conteúdo' na tabela de histórico — localizar uma linha cujo valor na coluna 'Ação realizada' seja exatamente 'Indexação de conteúdo'. [REVIEW_NEEDED: este step pressupõe que já existem registros de indexação gerados previamente no ambiente de stage; caso a tabela esteja vazia, o teste deve ser executado somente após disparar uma indexação.]
    - expect: Existe ao menos uma linha na tabela com 'Ação realizada' = 'Indexação de conteúdo'
  ...
  5. Clicar novamente no cabeçalho da coluna 'Ação realizada' para inverter para ordem decrescente (Z→A). Aguardar a resposta da tabela.
    ...
    - expect: Os dados das outras colunas de cada linha permanecem consistentes com seus respectivos registros
  6. [REVIEW_NEEDED] Verificar que registros gerados por um ambiente herdado (com toggle 'Herdar configurações do principal' ativo, ex.: envId 36799 conforme recon da aba Configurações) aparecem no histórico do Extrato do ambiente principal (org 36602). Este step requer: (a) confirmar que o ambiente alvo está configurado como herdado na aba Configurações, (b) confirmar que uma indexação foi disparada nesse sub-ambiente, (c) verificar que o registro aparece na tabela do ambiente principal. Dependência: dados de teste pré-existentes no stage ou fixture de setup dedicada.
    - expect: Ao menos um registro da tabela reflete consumo originado por um ambiente herdado
