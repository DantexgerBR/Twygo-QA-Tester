// spec: specs/configurar-indexacao-conteudo-por-ambiente/configurar-a-indexacao-trilha.plan.md
...
import { INHERITED_EDIT_BLOCK_TOOLTIP, SYNC_ALERT_TEXT } from '../../../utils/testIds.js';
...
test.describe('Configurar a utilização do indexação de conteúdo por ambiente', () => {
  test('Configurar a indexação - TRILHA', async ({ page }) => {
    ...
    await allure.story('Configurar a indexação - TRILHA');
    ...
    // ─── STEP 1: Habilitar toggle Indexação + marcar TRILHA + Salvar + verificar persistência ───
    await allure.step(
      '1. Habilitar toggle Indexação + marcar checkbox TRILHA + Salvar → modal RN37 + verificar persistência',
      async () => {
        ...
        if (syncBlocking) {
          ...
          // REVISAR: sync em andamento bloqueia interação — typeTrail disabled, não é possível marcar
        } else {
          ...
          // Marcar checkbox Trilha se ainda não estiver marcado
          ...
          // Modal RN37 só aparece se houver mudanças efetivas (toggle/checkbox que mudou estado).
          // Se checkbox Trilha já estava marcado pré-teste, save não dispara modal — apenas navega.
        ...
        // REVIEW_NEEDED: validação semântica "considera todas as trilhas e conteúdos vinculados"
        // requer validação de IA fim-a-fim e não pode ser automatizada somente via UI
      },
    );
    ...
    await allure.step(
      ...
      async () => {
        ...
        // os conteúdos corretamente requer teste funcional no chat IA do aluno — fora do escopo Admin UI.
      },
    );
    ...
    await allure.step(
      ...
      async () => {
        ...
        // configurações do principal requer teste funcional no chat IA do aluno — fora do escopo Admin UI.
      },
    );
  });
});
