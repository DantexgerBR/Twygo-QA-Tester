// spec: specs/visualizar-historico-de-consumo-da-indexacao-e-do-agente-de-atendimento/historico-do-agente-de-atendimento.plan.md
...
test.describe('Visualizar histórico de consumo da indexação e do agente de atendimento', () => {
  test('Histórico do agente de atendimento', async ({ page }) => {
    ...
    await allure.story('Histórico do agente de atendimento');
    ...
    // Step 3: Verificar registro de 'agente de atendimento' na tabela
    // REVISAR: este step pressupõe que já existem registros de agente de atendimento no ambiente de stage.
    // Caso a tabela esteja vazia ou sem registros do agente de atendimento, executar somente após disparar uma interação com o agente.
    await allure.step('3. Verificar existência de ao menos um registro de agente de atendimento na tabela', async () => {
      // REVISAR: requer fixture de dados — depende de registros pré-existentes no stage
      const tableContainer = page.getByTestId('ai-consumption-analysis-extract-table-container');
      const agenteRow = tableContainer.getByRole('row').filter({ hasText: 'agente de atendimento' }).first();
      await expect(agenteRow).toBeVisible();

      // Verificar que a coluna Data exibe data e hora no formato PT-BR
      const dateCell = agenteRow.locator('td').nth(0);
      await expect(dateCell).toHaveText(/\d{2}\/\d{2}\/\d{4}/);

      // Verificar que demais colunas não estão vazias
      const userCell = agenteRow.locator('td').nth(2);
      const creditsCell = agenteRow.locator('td').nth(3);
      const opCell = agenteRow.locator('td').nth(4);
    ...
    // REVISAR: requer (a) ambiente configurado como herdado, (b) interação com agente de atendimento disparada no sub-ambiente,
    ...
    await allure.step('6. [REVIEW_NEEDED] Verificar registros de ambiente herdado no extrato do principal', async () => {
      // REVISAR: requer fixture de dados — ambiente herdado (ex.: envId 36799) com interação do agente de atendimento disparada
      // Implementação: confirmar toggle "Herdar configurações do principal" ativo, disparar interação com agente,
    ...
    // REVISAR: requer (a) ambiente independente configurado, (b) interação com agente de atendimento disparada nele,
    ...
    await allure.step('7. [REVIEW_NEEDED] Verificar registros de ambiente independente aparecem somente no próprio extrato', async () => {
      // REVISAR: requer fixture de dados — ambiente independente com interação do agente de atendimento disparada
      // Implementação: confirmar toggle "Herdar configurações do principal" inativo, disparar interação com agente,
