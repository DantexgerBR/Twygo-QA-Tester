...
_11 caso(s) — 8 aprovado(s), 3 falha(s)_

### ✅ Aprovado · TC11 · Validar acessibilidade por teclado aba 'Layout' · ⚪ Menor

<a id="validar-acessibilidade-por-teclado-aba-layout"></a>_Arquivo:_ `acessibilidade-teclado-layout.spec.ts` · _Duração:_ 85.25s · _Browser:_ chromium
...
|---|---|---|:---:|---|---:|
| 1 | Acessar aba 'Layout' | Configurações das abas são carregadas | ✅ | — | 78.03s |
| 2 | Pressionar a tecla 'Tab' repetidamente | Foco percorre os campos em ordem | ✅ | — | 0.72s |
| 3 | Pressionar a tecla 'Enter' no botão '+ Adicionar Widget' com foco | Sistema redireciona para adição de widget | ✅ | — | 0.59s |
| 4 | Pressionar a tecla 'Esc' na tela aberta | Sistema retorna para o estado vazio ou abas criadas | ✅ | — | 0.45s |
...
- 📦 **Trace** — [`trace.zip`](artifacts/projects-widgets-tests-fea-f5690-ade-por-teclado-aba-Layout--chromium/trace.zip)
  ...
  ```bash
  npx playwright show-trace outputs/widgets/reports/all-suites_20260514-095639/artifacts/projects-widgets-tests-fea-f5690-ade-por-teclado-aba-Layout--chromium/trace.zip
  ```
...
<a id="ativar-switch-permitir-reorganizar-widgets-e-mover-widget"></a>_Arquivo:_ `ativar-switch-reorganizar-mover-widget.spec.ts` · _Duração:_ 82.20s · _Browser:_ chromium
...
| # | Ação do passo | Resultado esperado | Status | Notas | Duração |
|---|---|---|:---:|---|---:|
| 1 | Acessar a aba 'Layouts' do painel em edição no modo Desktop | Layout é exibido com os widgets | ✅ | — | 71.29s |
| 2 | Ativar o switch 'Permitir reorganizar widgets' | Switch fica ligado e widgets exibem indicação visual de que são arrastáveis | ✅ | — | 0.94s |
| 3 | Arrastar o primeiro widget para a posição do segundo | Widgets trocam de posição no grid | ✅ | — | 1.15s |
...
- 📦 **Trace** — [`trace.zip`](artifacts/projects-widgets-tests-fea-66a07-izar-widgets-e-mover-widget-chromium/trace.zip)
  ...
  ```bash
  npx playwright show-trace outputs/widgets/reports/all-suites_20260514-095639/artifacts/projects-widgets-tests-fea-66a07-izar-widgets-e-mover-widget-chromium/trace.zip
  ```
...
### ❌ Falhou · TC9 · Cancelar edição com alterações não salvas · 🟡 Normal

<a id="cancelar-edicao-com-alteracoes-nao-salvas"></a>_Arquivo:_ `cancelar-edicao-com-alteracoes.spec.ts` · _Duração:_ 228.76s · _Browser:_ chromium

> **❌ Por que falhou:** Não foi possível clicar o elemento — ele não ficou disponível em 30s (locator: getByTestId('panel-layout-cancel-button')).
> _Step impactado:_ **2. Clicar 'Cancelar' e capturar beforeunload dialog nativo (dismiss)**
...
| # | Ação do passo | Resultado esperado | Status | Notas | Duração |
|---|---|---|:---:|---|---:|
| 1 | Estar na aba 'Layouts' com alterações não salvas | Layout é exibido com alterações pendentes | ✅ | — | 77.64s |
| 2 | Clicar no botão 'Cancelar' da barra de rodapé | Sistema descarta alterações e retorna para a listagem de painéis | ❌ | Não foi possível clicar o elemento — ele não ficou disponível em 30s (locator: getByTestId('panel-layout-cancel-button')). | 30.12s |
...
- 📦 **Trace** — [`trace.zip`](artifacts/projects-widgets-tests-fea-c08c6-o-com-alterações-não-salvas-chromium/trace.zip)
  ...
  ```bash
  npx playwright show-trace outputs/widgets/reports/all-suites_20260514-095639/artifacts/projects-widgets-tests-fea-c08c6-o-com-alterações-não-salvas-chromium/trace.zip
  ```

- 📄 **DOM no momento da falha** — [`error-context.md`](artifacts/projects-widgets-tests-fea-c08c6-o-com-alterações-não-salvas-chromium/error-context.md)

#### 🐛 Pronto para registro de bug

**Descrição do BUG:** [Normal] Cancelar edição com alterações não salvas — Não foi possível clicar o elemento — ele não ficou disponível em 30s (locator: getByTestId('panel-layout-cancel-button')).

**Passo a passo para reprodução:**
1. Pré: Ambiente Stage configurado Funcionalidade 'Gestão de Painéis' habilitada no contrato Feature flag 'habilitar_paineis_do_usuario' ativa Usuário logado como Admin Painel em edição com alterações não salvas
1. 1. Estar na aba 'Layouts' com alterações não salvas
1. 2. Clicar no botão 'Cancelar' da barra de rodapé

**Comportamento esperado:** Sistema descarta alterações e retorna para a listagem de painéis

**Comportamento atual:** Não foi possível clicar o elemento — ele não ficou disponível em 30s (locator: getByTestId('panel-layout-cancel-button')).
...
| Outros | runId: all-suites_20260514-095639 |
| Outros | Duração até a falha: 228.76s |
| Outros | Step impactado: 2. 2. Clicar 'Cancelar' e capturar beforeunload dialog nativo (dismiss) |

**Evidências:**

- Screenshot capturado pelo Playwright (step-01-1-Setup-painel-com-1-widget-criando-altera-o-n-o-salva): artifacts/projects-widgets-tests-fea-c08c6-o-com-alterações-não-salvas-chromium/step-01-1-Setup-painel-com-1-widget-criando-altera-o-n-o-salva-c37524779dd363f6f1bb2933982cb80abeabefe9.png
- Snapshot do DOM/contexto da página (error-context.md): artifacts/projects-widgets-tests-fea-c08c6-o-com-alterações-não-salvas-chromium/error-context.md
- Trace completo do Playwright (.zip) — `npx playwright show-trace`: artifacts/projects-widgets-tests-fea-c08c6-o-com-alterações-não-salvas-chromium/trace.zip

<details><summary>📋 Ver texto agregado para copiar (Jira/GitHub/Linear)</summary>

```
Descrição do BUG
[Normal] Cancelar edição com alterações não salvas — Não foi possível clicar o elemento — ele não ficou disponível em 30s (locator: getByTestId('panel-layout-cancel-button')).

Passo a passo para reprodução
  Pré: Ambiente Stage configurado Funcionalidade 'Gestão de Painéis' habilitada no contrato Feature flag 'habilitar_paineis_do_usuario' ativa Usuário logado como Admin Painel em edição com alterações não salvas
  1. Estar na aba 'Layouts' com alterações não salvas
  2. Clicar no botão 'Cancelar' da barra de rodapé

Comportamento esperado
Sistema descarta alterações e retorna para a listagem de painéis

Comportamento atual
Não foi possível clicar o elemento — ele não ficou disponível em 30s (locator: getByTestId('panel-layout-cancel-button')).
...
- Outros:
  - Browser: chromium
  - runId: all-suites_20260514-095639
  - Duração até a falha: 228.76s
  - Step impactado: 2. 2. Clicar 'Cancelar' e capturar beforeunload dialog nativo (dismiss)
...
- testcase: Cancelar edição com alterações não salvas
...
<a id="estado-vazio-da-aba-sem-widgets"></a>_Arquivo:_ `estado-vazio-sem-widgets.spec.ts` · _Duração:_ 86.47s · _Browser:_ chromium
...
| # | Ação do passo | Resultado esperado | Status | Notas | Duração |
|---|---|---|:---:|---|---:|
| 1 | Acessar a aba 'Layouts' do painel em edição | Aba 'Nova aba' é exibida sem widgets | ✅ | — | 80.43s |
| 2 | Verificar o estado vazio da área de layout | Ícone ilustrativo centralizado é exibido, título "Nenhum widget adicionado", descrição "Clique no botão "Adicionar Widget" para começar a montar sua tela personalizada" e botão primário "+ Adicionar Widget" | ✅ | — | 0.29s |
| 3 | Clicar no botão '+ Adicionar Widget' do estado vazio | Drawer 'Widgets disponíveis' é aberto (mesma ação do botão da toolbar) | ✅ | — | 1.00s |
...
- 📦 **Trace** — [`trace.zip`](artifacts/projects-widgets-tests-fea-e27f8-do-vazio-da-aba-sem-widgets-chromium/trace.zip)
  ...
  ```bash
  npx playwright show-trace outputs/widgets/reports/all-suites_20260514-095639/artifacts/projects-widgets-tests-fea-e27f8-do-vazio-da-aba-sem-widgets-chromium/trace.zip
  ```
...
<a id="salvar-layout-pela-barra-de-rodape"></a>_Arquivo:_ `salvar-layout-rodape.spec.ts` · _Duração:_ 81.08s · _Browser:_ chromium
...
| # | Ação do passo | Resultado esperado | Status | Notas | Duração |
|---|---|---|:---:|---|---:|
| 1 | Acessar a aba 'Layouts' do painel em edição | Layout é exibido com o widget | ✅ | — | 53.25s |
| 2 | Clicar no botão 'Salvar layout' da barra de rodapé | Layout é persistido, sistema redireciona para a listagem de painéis e o painel aparece na listagem | ✅ | — | 0.78s |
| 3 | Reabrir o painel salvo | Aba e widgets continuam exibidos exatamente como salvos | ❌ | Error: expect(locator).toHaveCount(expected) failed | 20.78s |
...
- 📦 **Trace** — [`trace.zip`](artifacts/projects-widgets-tests-fea-7f9ad-layout-pela-barra-de-rodapé-chromium/trace.zip)
  ...
  ```bash
  npx playwright show-trace outputs/widgets/reports/all-suites_20260514-095639/artifacts/projects-widgets-tests-fea-7f9ad-layout-pela-barra-de-rodapé-chromium/trace.zip
  ```
...
| Outros | Browser: chromium |
| Outros | runId: all-suites_20260514-095639 |
| Outros | Duração até a falha: 81.08s |
...
<a id="switch-desativado-tentar-arrastar-widget"></a>_Arquivo:_ `switch-desativado-tentar-arrastar.spec.ts` · _Duração:_ 60.83s · _Browser:_ chromium
...
| # | Ação do passo | Resultado esperado | Status | Notas | Duração |
|---|---|---|:---:|---|---:|
| 1 | Acessar a aba 'Layouts' do painel em edição no modo Desktop | Layout é exibido com os widgets | ✅ | — | 52.35s |
| 2 | Manter o switch 'Permitir reorganizar widgets' desligado | Switch permanece desligado | ✅ | — | 1.15s |
| 3 | Tentar arrastar um widget | Widget não é movido (drag bloqueado) e nenhuma alteração ocorre no layout | ✅ | — | 0.56s |
...
- 📦 **Trace** — [`trace.zip`](artifacts/projects-widgets-tests-fea-c08e5-vado-tentar-arrastar-widget-chromium/trace.zip)
  ...
  ```bash
  npx playwright show-trace outputs/widgets/reports/all-suites_20260514-095639/artifacts/projects-widgets-tests-fea-c08e5-vado-tentar-arrastar-widget-chromium/trace.zip
  ```
...
### ❌ Falhou · TC10 · Toolbar permanece fixa ao rolar a área de layout · ⚪ Menor

<a id="toolbar-permanece-fixa-ao-rolar-a-area-de-layout"></a>_Arquivo:_ `toolbar-sticky-rolar.spec.ts` · _Duração:_ 38.80s · _Browser:_ chromium

> **❌ Por que falhou:** TimeoutError: page.goto: Timeout 30000ms exceeded.
> _Step impactado:_ **1. Setup — painel com 4 widgets para gerar altura suficiente pra scroll**
...
| # | Ação do passo | Resultado esperado | Status | Notas | Duração |
|---|---|---|:---:|---|---:|
| 1 | Acessar a aba 'Layouts' do painel em edição | Layout é exibido com vários widgets | ❌ | TimeoutError: page.goto: Timeout 30000ms exceeded. | 30.08s |
| 2 | Rolar verticalmente a área de layout | Barra de ferramentas permanece visível e fixa no topo durante a rolagem | ⊘ | Step não executado: o teste foi interrompido antes. Causa: TimeoutError: page.goto: Timeout 30000ms exceeded.. | — |
| 3 | Rolar verticalmente até o final | Barra de rodapé com 'Cancelar' e 'Salvar layout' permanece visível e fixa no rodapé | ⊘ | Step não executado: o teste foi interrompido antes. Causa: TimeoutError: page.goto: Timeout 30000ms exceeded.. | — |
...
- 📸 **Screenshot (screenshot)** — [`test-failed-1.png`](artifacts/projects-widgets-tests-fea-960fe-a-ao-rolar-a-área-de-layout-chromium/test-failed-1.png)

  ![](artifacts/projects-widgets-tests-fea-960fe-a-ao-rolar-a-área-de-layout-chromium/test-failed-1.png)
...
- 📦 **Trace** — [`trace.zip`](artifacts/projects-widgets-tests-fea-960fe-a-ao-rolar-a-área-de-layout-chromium/trace.zip)
  ...
  ```bash
  npx playwright show-trace outputs/widgets/reports/all-suites_20260514-095639/artifacts/projects-widgets-tests-fea-960fe-a-ao-rolar-a-área-de-layout-chromium/trace.zip
  ```
...
- 📄 **DOM no momento da falha** — [`error-context.md`](artifacts/projects-widgets-tests-fea-960fe-a-ao-rolar-a-área-de-layout-chromium/error-context.md)

#### 🐛 Pronto para registro de bug

**Descrição do BUG:** [Menor] Toolbar permanece fixa ao rolar a área de layout — TimeoutError: page.goto: Timeout 30000ms exceeded.

**Passo a passo para reprodução:**
1. Pré: Ambiente Stage configurado Funcionalidade 'Gestão de Painéis' habilitada no contrato Feature flag 'habilitar_paineis_do_usuario' ativa Usuário logado como Admin Painel em edição com aba contendo muitos widgets
1. 1. Acessar a aba 'Layouts' do painel em edição
1. 2. Rolar verticalmente a área de layout
1. 3. Rolar verticalmente até o final

**Comportamento esperado:** Layout é exibido com vários widgets

**Comportamento atual:** TimeoutError: page.goto: Timeout 30000ms exceeded.

**Informações:**

| Campo | Valor |
|---|---|
| URL | `https://widgets.stage.twygoead.com/o/36988/panels/new` |
| Login | ${TWYGO_STAGING_WIDGETS_EMAIL} |
| Senha | ${TWYGO_STAGING_WIDGETS_PASSWORD} |
| orgId | 36988 |
| Outros | Browser: chromium |
...
| Outros | Step impactado: 1. 1. Setup — painel com 4 widgets para gerar altura suficiente pra scroll |

**Evidências:**

- Screenshot capturado pelo Playwright (screenshot): artifacts/projects-widgets-tests-fea-960fe-a-ao-rolar-a-área-de-layout-chromium/test-failed-1.png
- Vídeo da execução (video.webm): artifacts/projects-widgets-tests-fea-960fe-a-ao-rolar-a-área-de-layout-chromium/video.webm
- Vídeo da execução (video-1.webm): artifacts/projects-widgets-tests-fea-960fe-a-ao-rolar-a-área-de-layout-chromium/video-1.webm
- Snapshot do DOM/contexto da página (error-context.md): artifacts/projects-widgets-tests-fea-960fe-a-ao-rolar-a-área-de-layout-chromium/error-context.md
- Trace completo do Playwright (.zip) — `npx playwright show-trace`: artifacts/projects-widgets-tests-fea-960fe-a-ao-rolar-a-área-de-layout-chromium/trace.zip

<details><summary>📋 Ver texto agregado para copiar (Jira/GitHub/Linear)</summary>

```
Descrição do BUG
[Menor] Toolbar permanece fixa ao rolar a área de layout — TimeoutError: page.goto: Timeout 30000ms exceeded.

Passo a passo para reprodução
  Pré: Ambiente Stage configurado Funcionalidade 'Gestão de Painéis' habilitada no contrato Feature flag 'habilitar_paineis_do_usuario' ativa Usuário logado como Admin Painel em edição com aba contendo muitos widgets
  1. Acessar a aba 'Layouts' do painel em edição
  2. Rolar verticalmente a área de layout
  3. Rolar verticalmente até o final
...
- Outros:
  ...
  - Step impactado: 1. 1. Setup — painel com 4 widgets para gerar altura suficiente pra scroll
...
- environment.json: staging-widgets
- testsuite: Layout das abas
- testcase: Toolbar permanece fixa ao rolar a área de layout
...
<a id="trocar-visualizacao-para-mobile-360-e-validar-alerta"></a>_Arquivo:_ `trocar-visualizacao-mobile.spec.ts` · _Duração:_ 63.12s · _Browser:_ chromium
...
| # | Ação do passo | Resultado esperado | Status | Notas | Duração |
|---|---|---|:---:|---|---:|
| 1 | Acessar a aba 'Layouts' do painel em edição | Layout é exibido no modo Desktop<br /> Com switch 'Permitir reorganizar widgets' habilitado | ✅ | — | 56.28s |
| 2 | Clicar no controle 'Visualização: Mobile' | Layout é renderizado em 360px e alerta exibido: 'Visualização (Mobile) — edição disponível no Desktop' | ✅ | — | 0.64s |
| 3 | Verificar o switch 'Permitir reorganizar widgets' | Switch fica desabilitado e exibe tooltip 'Edição disponível apenas no modo Desktop' | ✅ | — | 0.21s |
...
- 📦 **Trace** — [`trace.zip`](artifacts/projects-widgets-tests-fea-32f8d-Mobile-360-e-validar-alerta-chromium/trace.zip)
  ...
  ```bash
  npx playwright show-trace outputs/widgets/reports/all-suites_20260514-095639/artifacts/projects-widgets-tests-fea-32f8d-Mobile-360-e-validar-alerta-chromium/trace.zip
  ```
...
<a id="trocar-visualizacao-para-tablet-768-e-validar-alerta"></a>_Arquivo:_ `trocar-visualizacao-tablet.spec.ts` · _Duração:_ 65.00s · _Browser:_ chromium
...
| # | Ação do passo | Resultado esperado | Status | Notas | Duração |
|---|---|---|:---:|---|---:|
| 1 | Acessar a aba 'Layouts' do painel em edição | Layout é exibido no modo Desktop por padrão<br /> Com switch 'Permitir reorganizar widgets' habilitado | ✅ | — | 58.53s |
| 2 | Clicar no controle 'Visualização: Tablet' | Layout é renderizado em 768px e alerta exibido: 'Visualização (Tablet) — edição disponível no Desktop' | ✅ | — | 0.82s |
| 3 | Verificar o switch 'Permitir reorganizar widgets' | Switch fica desabilitado e exibe tooltip 'Edição disponível apenas no modo Desktop' ao passar o cursor | ✅ | — | 0.24s |
...
- 📦 **Trace** — [`trace.zip`](artifacts/projects-widgets-tests-fea-06fd1-Tablet-768-e-validar-alerta-chromium/trace.zip)
  ...
  ```bash
  npx playwright show-trace outputs/widgets/reports/all-suites_20260514-095639/artifacts/projects-widgets-tests-fea-06fd1-Tablet-768-e-validar-alerta-chromium/trace.zip
  ```
...
<a id="validar-barra-de-ferramentas-fixa-no-topo-da-area-de-layout"></a>_Arquivo:_ `validar-toolbar-fixa.spec.ts` · _Duração:_ 55.85s · _Browser:_ chromium
...
| # | Ação do passo | Resultado esperado | Status | Notas | Duração |
|---|---|---|:---:|---|---:|
| 1 | Acessar a aba 'Layouts' do painel em edição | Barra de ferramentas é exibida no topo da área de layout, fixa enquanto rola | ✅ | — | 50.74s |
| 2 | Verificar os elementos da toolbar | Botão '+ Adicionar widget' à esquerda; controles 'Visualização: Desktop', 'Visualização: Tablet', 'Visualização: Mobile' e switch 'Permitir reorganizar widgets' à direita | ✅ | — | 0.58s |
...
- 📦 **Trace** — [`trace.zip`](artifacts/projects-widgets-tests-fea-99416-a-no-topo-da-área-de-layout-chromium/trace.zip)
  ...
  ```bash
  npx playwright show-trace outputs/widgets/reports/all-suites_20260514-095639/artifacts/projects-widgets-tests-fea-99416-a-no-topo-da-área-de-layout-chromium/trace.zip
  ```
...
<a id="voltar-para-visualizacao-desktop-apos-tablet-mobile"></a>_Arquivo:_ `voltar-desktop.spec.ts` · _Duração:_ 42.01s · _Browser:_ chromium
...
| # | Ação do passo | Resultado esperado | Status | Notas | Duração |
|---|---|---|:---:|---|---:|
| 1 | Acessar a aba 'Layouts' do painel em edição | Layout é exibido | ✅ | — | 36.59s |
| 2 | Clicar no controle 'Visualização: Tablet' | Layout é renderizado em 768px | ✅ | — | 0.25s |
| 3 | Clicar no controle 'Visualização: Desktop' | Layout é renderizado em largura Desktop e alerta de Tablet/Mobile NÃO é exibido | ✅ | — | 0.44s |
| 4 | Verificar o switch 'Permitir reorganizar widgets' | Switch fica habilitado novamente | ✅ | — | 0.19s |
...
- 📦 **Trace** — [`trace.zip`](artifacts/projects-widgets-tests-fea-0fd86--Desktop-após-Tablet-Mobile-chromium/trace.zip)
  ...
  ```bash
  npx playwright show-trace outputs/widgets/reports/all-suites_20260514-095639/artifacts/projects-widgets-tests-fea-0fd86--Desktop-após-Tablet-Mobile-chromium/trace.zip
  ```
