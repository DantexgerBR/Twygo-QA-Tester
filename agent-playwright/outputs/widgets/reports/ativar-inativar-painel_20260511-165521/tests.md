...
_5 caso(s) — 2 aprovado(s), 2 falha(s), 1 ignorado(s)_

### ✅ Aprovado · Ativar um painel previamente inativo · 🔴 Crítico

<a id="ativar-um-painel-previamente-inativo"></a>_Arquivo:_ `ativar-painel-inativo.spec.ts` · _Duração:_ 11.63s · _Browser:_ chromium
...
|---|---|---|:---:|---|---:|
| 1 | Acessar a aba 'Painéis' em Configurações > Menu | Listagem é exibida com 'Painel QA Teste' inativo | ✅ | — | 8.20s |
...
<a id="tentar-inativar-painel-associado-a-modos-de-uso-exibe-modal-de-bloqueio"></a>_Arquivo:_ `inativar-painel-associado-modal.spec.ts` · _Duração:_ 20.01s · _Browser:_ chromium
...
> _Step impactado:_ **2. Clicar no switch "Ativo" da linha "Painel Vinculado TC3 w1-1778529235400" e verificar modal "Painel em uso"**
...
| # | Ação do passo | Resultado esperado | Status | Notas | Duração |
|---|---|---|:---:|---|---:|
| — | _Pré-condição:_ 1. Acessar a aba "Painéis" com o painel vinculado ativo | — | ✅ | — | 7.55s |
| — | _Pré-condição:_ 2. Clicar no switch "Ativo" da linha "Painel Vinculado TC3 w1-1778529235400" e verificar modal "Painel em uso" | — | ❌ | O elemento esperado não apareceu na tela (locator: locator('.chakra-modal__content').filter({ has: locator('[data-test-id="panel-in-use-modal-confirm"]') })). | 10.08s |
...
| Outros | runId: ativar-inativar-painel_20260511-165521 |
| Outros | Duração até a falha: 20.01s |
| Outros | Step impactado: 2. 2. Clicar no switch "Ativo" da linha "Painel Vinculado TC3 w1-1778529235400" e verificar modal "Painel em uso" |
...
- Outros:
  - Browser: chromium
  - runId: ativar-inativar-painel_20260511-165521
  - Duração até a falha: 20.01s
  - Step impactado: 2. 2. Clicar no switch "Ativo" da linha "Painel Vinculado TC3 w1-1778529235400" e verificar modal "Painel em uso"
...
<a id="inativar-um-painel-nao-associado-a-nenhum-modo-de-uso"></a>_Arquivo:_ `inativar-painel-nao-associado.spec.ts` · _Duração:_ 16.59s · _Browser:_ chromium
...
| # | Ação do passo | Resultado esperado | Status | Notas | Duração |
|---|---|---|:---:|---|---:|
| 1 | Acessar a aba 'Painéis' em Configurações > Menu | Listagem é exibida com 'Painel QA Teste' ativo | ✅ | — | 8.45s |
| 2 | Clicar no switch da coluna 'Ativo' na linha 'Painel QA Teste' | Switch fica desligado e estado 'Inativo' é persistido | ✅ | — | 1.13s |
| 3 | Recarregar a página | Painel 'Painel QA Teste' permanece com switch desligado na coluna 'Ativo' | ✅ | — | 4.52s |
...
<a id="verificar-persistencia-do-estado-ativo-apos-reload"></a>_Arquivo:_ `persistencia-estado-ativo-reload.spec.ts` · _Duração:_ 24.30s · _Browser:_ chromium

> **❌ Por que falhou:** O elemento esperado não apareceu na tela (locator: locator('tbody tr').filter({ has: locator('td:first-child p').filter({ hasText: 'Painel 30' }) })).
> _Step impactado:_ **1. Acessar a aba "Painéis" e capturar estado inicial**
...
| # | Ação do passo | Resultado esperado | Status | Notas | Duração |
|---|---|---|:---:|---|---:|
| 1 | Acessar a aba 'Painéis' em Configurações > Menu | Listagem é exibida | ❌ | O elemento esperado não apareceu na tela (locator: locator('tbody tr').filter({ has: locator('td:first-child p').filter({ hasText: 'Painel 30' }) })). | 21.34s |
| 2 | Alterar o switch 'Ativo' de um painel para o estado oposto | Switch reflete o novo estado | ⊘ | Step não executado: o teste foi interrompido antes. Causa: O elemento esperado não apareceu na tela (locator: locator('tbody tr').filter({ has: locator('td:first-child p').filter({ hasText: 'Painel 30' }) })).. | — |
| 3 | Recarregar a página (F5) | Listagem é recarregada e o switch mantém o estado alterado | ⊘ | Step não executado: o teste foi interrompido antes. Causa: O elemento esperado não apareceu na tela (locator: locator('tbody tr').filter({ has: locator('td:first-child p').filter({ hasText: 'Painel 30' }) })).. | — |

**Evidências:**

![screenshot](../../test-artifacts/projects-widgets-tests-fea-9043e-do-estado-Ativo-após-reload-chromium/test-failed-1.png)

- 📦 [Trace do Playwright (trace.zip)](../../test-artifacts/projects-widgets-tests-fea-9043e-do-estado-Ativo-após-reload-chromium/trace.zip) — abra com `npx playwright show-trace`

- 📄 [Snapshot do DOM no momento da falha (`error-context.md`)](../../test-artifacts/projects-widgets-tests-fea-9043e-do-estado-Ativo-após-reload-chromium/error-context.md)

#### 🐛 Pronto para registro de bug

**Descrição do BUG:** [Normal] Verificar persistência do estado Ativo após reload — O elemento esperado não apareceu na tela (locator: locator('tbody tr').filter({ has: locator('td:first-child p').filter({ hasText: 'Painel 30' }) })).
...
**Comportamento atual:** O elemento esperado não apareceu na tela (locator: locator('tbody tr').filter({ has: locator('td:first-child p').filter({ hasText: 'Painel 30' }) })).
...
| Outros | Browser: chromium |
| Outros | runId: ativar-inativar-painel_20260511-165521 |
| Outros | Duração até a falha: 24.30s |
| Outros | Step impactado: 1. 1. Acessar a aba "Painéis" e capturar estado inicial |

**Evidências:**

- Screenshot capturado pelo Playwright (screenshot): ../../test-artifacts/projects-widgets-tests-fea-9043e-do-estado-Ativo-após-reload-chromium/test-failed-1.png
- Snapshot do DOM/contexto da página (error-context.md): ../../test-artifacts/projects-widgets-tests-fea-9043e-do-estado-Ativo-após-reload-chromium/error-context.md
- Trace completo do Playwright (.zip) — `npx playwright show-trace`: ../../test-artifacts/projects-widgets-tests-fea-9043e-do-estado-Ativo-após-reload-chromium/trace.zip

<details><summary>📋 Ver texto agregado para copiar (Jira/GitHub/Linear)</summary>

```
Descrição do BUG
[Normal] Verificar persistência do estado Ativo após reload — O elemento esperado não apareceu na tela (locator: locator('tbody tr').filter({ has: locator('td:first-child p').filter({ hasText: 'Painel 30' }) })).
...
Comportamento atual
O elemento esperado não apareceu na tela (locator: locator('tbody tr').filter({ has: locator('td:first-child p').filter({ hasText: 'Painel 30' }) })).
...
- Outros:
  - Browser: chromium
  - runId: ativar-inativar-painel_20260511-165521
  - Duração até a falha: 24.30s
  - Step impactado: 1. 1. Acessar a aba "Painéis" e capturar estado inicial
...
Locator: locator('tbody tr').filter({ has: locator('td:first-child p').filter({ hasText: 'Painel 30' }) })
Expected: visible
Timeout: 10000ms
Error: element(s) not found
...
Call log:
  - Expect "toBeVisible" with timeout 10000ms
  - waiting for locator('tbody tr').filter({ has: locator('td:first-child p').filter({ hasText: 'Painel 30' }) })
...
<a id="tentar-reativar-menu-de-modo-de-uso-vinculado-a-painel-inativo-exibe-modal"></a>_Arquivo:_ `reativar-modo-uso-painel-inativo.spec.ts` · _Duração:_ 0.59s · _Browser:_ chromium
