...
<a id="ativar-um-painel-previamente-inativo"></a>_Arquivo:_ `ativar-painel-previamente-inativo.spec.ts` · _Duração:_ 10.97s · _Browser:_ chromium
...
|---|---|---|:---:|---|---:|
| 1 | Acessar a aba 'Painéis' em Configurações > Menu | Listagem é exibida com 'Painel QA Teste' inativo | ✅ | — | 6.63s |
| 2 | Clicar no switch da coluna 'Ativo' na linha 'Painel QA Teste' | Switch fica ligado e estado 'Ativo' é persistido | ✅ | — | 1.14s |
...
<a id="inativar-um-painel-nao-associado-a-nenhum-modo-de-uso"></a>_Arquivo:_ `inativar-painel-nao-associado.spec.ts` · _Duração:_ 18.03s · _Browser:_ chromium
...
| # | Ação do passo | Resultado esperado | Status | Notas | Duração |
|---|---|---|:---:|---|---:|
| 1 | Acessar a aba 'Painéis' em Configurações > Menu | Listagem é exibida com 'Painel QA Teste' ativo | ✅ | — | 7.00s |
| 2 | Clicar no switch da coluna 'Ativo' na linha 'Painel QA Teste' | Switch fica desligado e estado 'Inativo' é persistido | ✅ | — | 1.12s |
| 3 | Recarregar a página | Painel 'Painel QA Teste' permanece com switch desligado na coluna 'Ativo' | ✅ | — | 7.05s |
...
| Outros | runId: ativar-inativar-painel_20260512-143815 |
...
- Outros:
  - Browser: chromium
  - runId: ativar-inativar-painel_20260512-143815
...
<a id="tentar-reativar-modo-de-uso-vinculado-a-um-painel-inativo"></a>_Arquivo:_ `tentar-reativar-modo-uso-painel-inativo.spec.ts` · _Duração:_ 0.68s · _Browser:_ chromium

> **⊘ Por que foi ignorado:** Marcado para revisão (test.fixme): Seed ausente: cenário composto (painel inativo + menu de modo de uso vinculado ao mesmo painel + menu inativo) não tem auto-seed determinístico no env staging-widgets. Depende da correção prévia do bug GET /panels/{id}/linked_menus 500 (ver TC3 — tentar-inativar-painel-associado.spec.ts) para que a associação painel→menu seja verificável antes de inativar o painel. Destinatário: QA Lead — criar seed manual ou aguardar fix do TC3 para destravar.
...
| # | Ação do passo | Resultado esperado | Status | Notas | Duração |
|---|---|---|:---:|---|---:|
| 1 | Acessar Menu > Modos de uso | Tela de Modos de uso é exibida | ⊘ | Step não executado — Marcado para revisão (test.fixme): Seed ausente: cenário composto (painel inativo + menu de modo de uso vinculado ao mesmo painel + menu inativo) não tem auto-seed determinístico no env staging-widgets. Depende da correção prévia do bug GET /panels/{id}/linked_menus 500 (ver TC3 — tentar-inativar-painel-associado.spec.ts) para que a associação painel→menu seja verificável antes de inativar o painel. Destinatário: QA Lead — criar seed manual ou aguardar fix do TC3 para destravar. | — |
| 2 | Clicar no switch de ativação do menu vinculado a 'Painel QA Teste' | Modal informativo é exibido informando que o painel está inativo e não pode ser reativado | ⊘ | Step não executado — Marcado para revisão (test.fixme): Seed ausente: cenário composto (painel inativo + menu de modo de uso vinculado ao mesmo painel + menu inativo) não tem auto-seed determinístico no env staging-widgets. Depende da correção prévia do bug GET /panels/{id}/linked_menus 500 (ver TC3 — tentar-inativar-painel-associado.spec.ts) para que a associação painel→menu seja verificável antes de inativar o painel. Destinatário: QA Lead — criar seed manual ou aguardar fix do TC3 para destravar. | — |
| 3 | Clicar no botão 'Fechar' do modal | Modal é fechado e menu permanece inativo | ⊘ | Step não executado — Marcado para revisão (test.fixme): Seed ausente: cenário composto (painel inativo + menu de modo de uso vinculado ao mesmo painel + menu inativo) não tem auto-seed determinístico no env staging-widgets. Depende da correção prévia do bug GET /panels/{id}/linked_menus 500 (ver TC3 — tentar-inativar-painel-associado.spec.ts) para que a associação painel→menu seja verificável antes de inativar o painel. Destinatário: QA Lead — criar seed manual ou aguardar fix do TC3 para destravar. | — |
...
- **Motivo do skip / fixme:** Seed ausente: cenário composto (painel inativo + menu de modo de uso vinculado ao mesmo painel + menu inativo) não tem auto-seed determinístico no env staging-widgets. Depende da correção prévia do bug GET /panels/{id}/linked_menus 500 (ver TC3 — tentar-inativar-painel-associado.spec.ts) para que a associação painel→menu seja verificável antes de inativar o painel. Destinatário: QA Lead — criar seed manual ou aguardar fix do TC3 para destravar.
...
<a id="verificar-persistencia-do-estado-ativo-apos-reload"></a>_Arquivo:_ `verificar-persistencia-estado-ativo.spec.ts` · _Duração:_ 15.38s · _Browser:_ chromium
...
| # | Ação do passo | Resultado esperado | Status | Notas | Duração |
|---|---|---|:---:|---|---:|
| 1 | Acessar a aba 'Painéis' em Configurações > Menu | Listagem é exibida | ✅ | — | 6.70s |
| 2 | Alterar o switch 'Ativo' de um painel para o estado oposto | Switch reflete o novo estado | ✅ | — | 1.12s |
| 3 | Recarregar a página (F5) | Listagem é recarregada e o switch mantém o estado alterado | ✅ | — | 4.58s |
