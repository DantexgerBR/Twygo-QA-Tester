...
- Name: projects\registros-externos\tests\features\editar-registro-matriz-perfil-origem-status-banners\tc5-campo-pessoa-desabilitado-admin.spec.ts >> Editar registro de aprendizagem (matriz perfil × origem × status e banners) >> Validar campo Pessoa desabilitado na edição do Admin
- Location: projects\registros-externos\tests\features\editar-registro-matriz-perfil-origem-status-banners\tc5-campo-pessoa-desabilitado-admin.spec.ts:11:3
...
```ts
  ...
  6  | 
  7  | // DIVERGÊNCIA (recon 2026-06-22): o campo é rotulado "Pessoas*" (plural, obrigatório) e o
  8  | // input `people-selector-input` NÃO está disabled — diferente da RN43 (dono fixo, campo
  9  | // "Pessoa" desabilitado em admin-editar). Assere a expectativa da AT e falha vermelho.
  10 | test.describe(SUITE, () => {
  11 |   test('Validar campo Pessoa desabilitado na edição do Admin', async ({ page }) => {
  12 |     await allure.epic('Twygo - Registros de Aprendizagem');
  13 |     await allure.feature(SUITE);
  14 |     await allure.story('TC5 — campo Pessoa desabilitado (Admin)');
  ...
  25 |     });
  26 | 
  27 |     await allure.step('Campo Pessoa desabilitado (RN43)', async () => {
> 28 |       await expect(form.peopleInput()).toBeDisabled();
     |                                        ^ Error: expect(locator).toBeDisabled() failed
  29 |     });
  30 |   });
  31 | });
