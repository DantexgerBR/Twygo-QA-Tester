...
<a id="tc1-switch-habilitar-reinscricao-aparece-com-flag-on-na-edicao-de-curso"></a>_Arquivo:_ `tc1-switch-aparece-com-flag-on.spec.ts` · _Duração:_ 75.75s · _Browser:_ chromium

> **❌ Por que falhou:** Não foi possível clicar o elemento — ele não ficou disponível em 30s (locator: locator('[data-test-id="tab-access"]').or(getByRole('tab', { name: /^Acesso$/i })).first()).
> _Step impactado:_ **4. Switch "Habilitar reinscrição" está visível no formulário (tab "Acesso" do facelift)**
...
|---|---|---|:---:|---|---:|
| 1 | Acessar a URL "/o/{orgId}/dashboard" | Dashboard padrão é exibido contendo o menu lateral. | ✅ | — | 10.15s |
| 2 | Navegar até a listagem de cursos da organização | Listagem de cursos é exibida com pelo menos 1 curso pré-existente. | ✅ | — | 10.81s |
| 3 | Clicar no botão `more_vert` da linha do curso → "Gerenciar" | Página de edição do curso é exibida na rota `/o/{orgId}/contents/{id}/edit?tab=identification` (aba "Identificação" ativa por default). | ✅ | — | 19.96s |
| 4 | Clicar na aba "Acesso" do formulário de edição | URL atualiza para `?tab=access` e tabpanel "Acesso" renderiza, contendo a seção "Inscrição" → "Permitir registro de inscrição por". | ❌ | Não foi possível clicar o elemento — ele não ficou disponível em 30s (locator: locator('[data-test-id="tab-access"]').or(getByRole('tab', { name: /^Acesso$/i })).first()). | 30.45s |
| 5 | Localizar o checkbox "Habilitar reinscrição" dentro da seção "Permitir registro de inscrição por" | Checkbox "Habilitar reinscrição" está visível no formulário (controle HTML padrão, não Chakra switch). | ⊘ | Step não executado: o teste foi interrompido antes. Causa: Não foi possível clicar o elemento — ele não ficou disponível em 30s (locator: locator('[data-test-id="tab-access"]').or(getByRole('tab', { name: /^Acesso$/i })).first()).. | — |
...
- 📦 **Trace** — [`trace.zip`](artifacts/projects-recertificacao-te-d835b--flag-ON-na-edição-de-curso-chromium/trace.zip)
  ...
  ```bash
  npx playwright show-trace outputs/recertificacao/reports/configuracao-de-conteudo-switch-habilitar-reinscricao_20260601-084412/artifacts/projects-recertificacao-te-d835b--flag-ON-na-edição-de-curso-chromium/trace.zip
  ```
...
**Descrição do BUG:** [Crítico] TC1 — Switch "Habilitar reinscrição" aparece com flag ON na edição de curso — Não foi possível clicar o elemento — ele não ficou disponível em 30s (locator: locator('[data-test-id="tab-access"]').or(getByRole('tab', { name: /^Acesso$/i })).first()).
...
**Comportamento esperado:** URL atualiza para `?tab=access` e tabpanel "Acesso" renderiza, contendo a seção "Inscrição" → "Permitir registro de inscrição por".

**Comportamento atual:** Não foi possível clicar o elemento — ele não ficou disponível em 30s (locator: locator('[data-test-id="tab-access"]').or(getByRole('tab', { name: /^Acesso$/i })).first()). (falha aconteceu no passo 4: "Switch "Habilitar reinscrição" está visível no formulário (tab "Acesso" do facelift)", que deveria resultar em: URL atualiza para `?tab=access` e tabpanel "Acesso" renderiza, contendo a seção "Inscrição" → "Permitir registro de inscrição por".)
...
| Outros | runId: configuracao-de-conteudo-switch-habilitar-reinscricao_20260601-084412 |
| Outros | Duração até a falha: 75.75s |
| Outros | Step impactado: 4. Switch "Habilitar reinscrição" está visível no formulário (tab "Acesso" do facelift) |
...
```
Descrição do BUG
[Crítico] TC1 — Switch "Habilitar reinscrição" aparece com flag ON na edição de curso — Não foi possível clicar o elemento — ele não ficou disponível em 30s (locator: locator('[data-test-id="tab-access"]').or(getByRole('tab', { name: /^Acesso$/i })).first()).
...
Comportamento atual
Não foi possível clicar o elemento — ele não ficou disponível em 30s (locator: locator('[data-test-id="tab-access"]').or(getByRole('tab', { name: /^Acesso$/i })).first()). (falha aconteceu no passo 4: "Switch "Habilitar reinscrição" está visível no formulário (tab "Acesso" do facelift)", que deveria resultar em: URL atualiza para `?tab=access` e tabpanel "Acesso" renderiza, contendo a seção "Inscrição" → "Permitir registro de inscrição por".)
...
- Outros:
  - Browser: chromium
  - runId: configuracao-de-conteudo-switch-habilitar-reinscricao_20260601-084412
  - Duração até a falha: 75.75s
  - Step impactado: 4. Switch "Habilitar reinscrição" está visível no formulário (tab "Acesso" do facelift)
...
<a id="tc3-ativar-e-salvar-o-switch-persiste-has-recertification-true"></a>_Arquivo:_ `tc3-ativar-e-salvar-persiste.spec.ts` · _Duração:_ 25.23s · _Browser:_ chromium
...
| # | Ação do passo | Resultado esperado | Status | Notas | Duração |
|---|---|---|:---:|---|---:|
| 1 | Acessar a edição de um curso com `has_recertification = false` | Página de edição é exibida e switch "Habilitar reinscrição" está visível e desligado. | ❌ | A condição esperada não se tornou verdadeira dentro do tempo limite — a UI não chegou ao estado aguardado. | 22.07s |
...
- 📦 **Trace** — [`trace.zip`](artifacts/projects-recertificacao-te-ccb33-e-has-recertification-true--chromium/trace.zip)
  ...
  ```bash
  npx playwright show-trace outputs/recertificacao/reports/configuracao-de-conteudo-switch-habilitar-reinscricao_20260601-084412/artifacts/projects-recertificacao-te-ccb33-e-has-recertification-true--chromium/trace.zip
  ```
...
| Outros | Browser: chromium |
| Outros | runId: configuracao-de-conteudo-switch-habilitar-reinscricao_20260601-084412 |
| Outros | Duração até a falha: 25.23s |
...
- 📦 **Trace** — [`trace.zip`](artifacts/projects-recertificacao-te-ad265-ritos-é-permitido-sem-aviso-chromium/trace.zip)
  ...
  ```bash
  npx playwright show-trace outputs/recertificacao/reports/configuracao-de-conteudo-switch-habilitar-reinscricao_20260601-084412/artifacts/projects-recertificacao-te-ad265-ritos-é-permitido-sem-aviso-chromium/trace.zip
  ```
