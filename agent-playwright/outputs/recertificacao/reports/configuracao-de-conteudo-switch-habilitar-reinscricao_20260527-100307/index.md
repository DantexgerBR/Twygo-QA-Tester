...
**Projeto:** Recertificação · **Ambiente:** `staging-recertificacao` · **Browsers:** chromium · **Gerado em:** 27/05/2026, 10:03:07
...
>
> - 🔴 Crítico [TC1 · Switch "Habilitar reinscrição" aparece com flag ON na edição de curso](tests.md#tc1-switch-habilitar-reinscricao-aparece-com-flag-on-na-edicao-de-curso) — A condição esperada não se tornou verdadeira dentro do tempo limite — a UI não chegou ao estado aguardado.
> - 🔴 Crítico [TC3 · Ativar e salvar o switch persiste `has_recertification = true`](tests.md#tc3-ativar-e-salvar-o-switch-persiste-has-recertification-true) — O elemento esperado não apareceu na tela (locator: getByRole('checkbox', { name: /Habilitar reinscrição/i })).
...
> - 🟡 Normal [TC5 · Paridade do switch entre formulário HAML e formulário React (facelift)](tests.md#tc5-paridade-do-switch-entre-formulario-haml-e-formulario-react-facelift) — O elemento esperado não apareceu na tela (locator: getByRole('checkbox', { name: /Habilitar reinscrição/i })).
...
|---|---:|---|---:|---:|---:|---|---:|
| **Configuração de Conteúdo (Switch "Habilitar reinscrição")** | 5 | `[✗✗✗✗✗✗✗✗✗✗✗✗✗✗✗✗⊘⊘⊘⊘]` 0/4/1 | 0% | 4 | 1 | — | 66.0s |
...
**Distribuição**: **2** spec-fragil · **2** inconclusivo
