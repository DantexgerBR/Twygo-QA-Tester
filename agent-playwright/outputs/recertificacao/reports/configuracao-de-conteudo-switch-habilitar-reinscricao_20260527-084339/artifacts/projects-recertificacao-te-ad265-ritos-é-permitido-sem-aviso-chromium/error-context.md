      ...
      - generic [ref=e471]:
        - generic [ref=e472]:
          - button "Voltar" [ref=e474] [cursor=pointer]:
            - img [ref=e476]
            - text: Voltar
          - heading "Curso Recertificação TC4 w2-1779882137242" [level=2] [ref=e480]
        - generic [ref=e483]:
          - tablist [ref=e484]:
                ...
                - generic [ref=e499]:
                  ...
                  - group [ref=e512]:
                    ...
                    - textbox "Nome *" [active] [ref=e518]:
                      ...
                      - text: Curso Recertificação TC4 w2-1779882137242
                  ...
                  - group [ref=e614]:
                    ...
                    - application "Editor de Rich Text, description" [ref=e619]:
                      ...
                      - iframe [ref=e669]:
                        - paragraph [ref=f19e2]: Seed automatizado — Curso Recertificação TC4 w2-1779882137242 (createCurso v1.3).
                  - group [ref=e671]:
...
> 272 |     await checkbox.waitFor({ state: 'visible', timeout: 10_000 });
      |                    ^ TimeoutError: locator.waitFor: Timeout 10000ms exceeded.
  273 |     if (enabled) {
  274 |       await checkbox.check();
  275 |     } else {
  276 |       await checkbox.uncheck();
  277 |     }
  278 |   }
  279 | 
  280 |   // ─── Submit / mensagens ─────────────────────────────────────────────
  281 | 
  282 |   getSaveButton(): Locator {
  283 |     // Tela HAML usa <input type="submit" value="Salvar">; React usa <button>Salvar</button>.
  284 |     // role=button cobre os dois.
  285 |     return this.page.getByRole('button', { name: /^Salvar$/ }).first();
  286 |   }
  287 | 
  288 |   async save(): Promise<void> {
  289 |     await this.getSaveButton().click();
  290 |   }
  291 | 
  292 |   /**
  293 |    * Aguarda confirmação de save bem-sucedido. Twygo usa toast Chakra +
  294 |    * redirect para a listagem (variando entre HAML/React). Asserta a
  295 |    * primeira condição que aparecer (toast OU URL de listagem).
  296 |    *
  297 |    * REVISAR-FIGMA: texto exato do toast de sucesso ainda não confirmado.
  298 |    */
  299 |   async expectSaveSuccess(): Promise<void> {
  300 |     // Toast Chakra de sucesso (status=success) — primeiro sinal pós-submit.
  301 |     const toast = this.page
  302 |       .locator('.chakra-toast, [role="status"]')
  303 |       .filter({ hasText: /salv|sucesso/i })
  304 |       .first();
  305 |     await expect
  306 |       .poll(async () => {
  307 |         const toastVisible = await toast.isVisible().catch(() => false);
  308 |         const url = this.page.url();
  309 |         return (
  310 |           toastVisible ||
  311 |           /\/o\/\d+\/events(\?|$)/.test(url) ||
  312 |           /\/(e\/\d+\/edit|contents\/\d+\/edit)/.test(url)
  313 |         );
  314 |       })
  315 |       .toBe(true);
  316 |   }
