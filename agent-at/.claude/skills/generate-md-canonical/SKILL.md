...
## Mapeamento prioridade ★ → frontmatter

| Antiga marca (XMind legado) | Novo valor `**Prioridade**` |
|---|---|
| `priority-1` / ★★★ | `critical` |
| `priority-2` / ★★ | `medium` (ou `high` para casos importantes mas não-críticos) |
| `priority-3` / ★ | `low` |

## Playbooks Twygo — tabela canônica

Slugs aceitos em `playbooks:` no frontmatter de suíte. Lista completa em
[CONTRACT.md §6](../../../../CONTRACT.md). Resumo:

| Slug | Quando declarar |
|---|---|
| `flipper` | TC depende de toggle de feature flag em runtime |
| `super-admin` | TC altera contrato/funcionalidade via Super Admin |
| `trial` | TC roda em org Trial |
| `ambientes-adicionais` | TC valida isolamento multi-tenant |
| `filtro-drawer` | Listagem com filtro via drawer Chakra |
| `toast-chakra` | TC valida toast Chakra |
| `switch-chakra` | TC interage com switch Chakra |
| `beforeunload` | TC clica "Cancelar" com form sujo |
| `perfil-switch` | TC valida visão de perfil diferente |
| `cleanup-dados` | TC cria/altera estado persistente |

Decidir quais playbooks declarar lendo as pré-condições e passos:
- Se cita "feature flag" → declarar `flipper`
- Se cita "Super Admin" / "contrato" → declarar `super-admin`
- Se cita "Trial" / "ICP Outros" → declarar `trial`
- Se UI usa drawer de filtro → declarar `filtro-drawer`
- Se valida toast Chakra → declarar `toast-chakra`
- Etc.

## Validação pós-geração

Após escrever o arquivo, validar:

1. **Parse com o parser oficial**:
   ```bash
   python scripts/md_canonical_parser.py projects/<slug>/output/test-analysis.md > /tmp/parsed.json
   ```
   Se o parser falhar, o MD tem erro de schema. Corrigir.

2. **Conferir totais**: o campo `totals` do frontmatter bate com o conteúdo real?

3. **Conferir frontmatter de suíte**: todas as suítes têm `suite`, `executor`, `preconditions`?

4. **Conferir passos**: cada passo tem formato `N. <ação>\n   → <resultado>`?

## Anti-patterns proibidos

Vide [agent-at/CLAUDE.md §8](../../../CLAUDE.md):

- **A**: Ações vagas ("verificar", "conferir")
- **B**: Resultados sem objeto ("sistema funciona")
- **C**: Combinar múltiplas ações num passo
- **D**: Referenciar passo anterior
- **E**: Pré-condição referenciando trabalho de dev
- **F**: Hardcode de credentials/orgIds reais
